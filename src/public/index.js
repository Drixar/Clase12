...
socketClient.on("messages",async (dataMsg)=>{
    ...
    dataOriginal = JSON.stringify(normalData, null, "\t").length;
    // console.log("data original", dataOriginal)
    dataNormalizada = JSON.stringify(dataMsg, null, "\t").length;
    // console.log("data normalizada", dataNormalizada);
    percCompressed = Math.round((dataNormalizada / dataOriginal) -1)
    // console.log("porcentaje", percCompressed)
    compression.innerHTML = `% compresion: ${percCompressed} %`
    let messageElements = "";
